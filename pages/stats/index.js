...
import { ButtonSegmentRow, Button, PrimaryButton } from '../../components/Button'
...
class Stats extends React.Component<{}, State> {
  ...
  componentDidMount = async () => {
    const stats = await this.getStats(this.state.range)
  ...
  formatStats = (stats: any) => {
    ...
    const networkTotal = stats.reduce((acc, curr) => {
    ...
    return raw.map(statObj => {
      const diff = (statObj.totals.current - statObj.totals.previous) / statObj.totals.previous
    ...
    return (
      <Page dataCy={'stats-view'}>
        <Head>
          ...
          <meta content={"Spec · Stats"} name="twitter:title" key="twitter:title" />
        </Head>
          ...
          <ButtonSegmentRow>
            {
              ['week', 'month', 'year', 'all'].map(r => {
                if (range === r) return (
                  <PrimaryButton key={r} onClick={this.changeRange(r)}>{capitalize(r)}</PrimaryButton>
                )

                return (
                  <Button onClick={this.changeRange(r)} key={r}>{capitalize(r)}</Button>
                )
              })
            }
          </ButtonSegmentRow>
